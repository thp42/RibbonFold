...
import numpy as np
from . import residue_constants, r3, utils
...
def atom37_to_torsion_angles(
    ...
    placeholder_for_undefined=False ) -> Dict[str, torch.Tensor]:
    ...
    aatype = torch.minimum(aatype, torch.tensor([20.0]).to(device))
    
    # Compute the backbone angles.
    num_batch, num_res = aatype.shape

    pad = torch.zeros([num_batch, 1, 37, 3]).to(aatype)
    prev_all_atom_pos = torch.cat([pad, all_atom_pos[:, :-1, :, :]], dim=1)

    pad = torch.zeros([num_batch, 1, 37]).to(aatype)
    ...
    atom_indices = utils.batched_gather(params=chi_atom_indices, indices=aatype, dim=0, batch_dims=0)
    # Gather atom positions. Shape: [batch, num_res, chis=4, atoms=4, xyz=3].
    chis_atom_pos = utils.batched_gather(params=all_atom_pos, indices=atom_indices, dim=-2, batch_dims=2)

    # Copy the chi angle mask, add the UNKNOWN residue. Shape: [restypes, 4].
    chi_angles_mask = list(residue_constants.chi_angles_mask)
    chi_angles_mask.append([0.0, 0.0, 0.0, 0.0])
    chi_angles_mask = torch.tensor(chi_angles_mask).to(aatype)

    # Compute the chi angle mask. I.e. which chis angles exist according to the
    # aatype. Shape [batch, num_res, chis=4].
    chis_mask = utils.batched_gather(params=chi_angles_mask, indices=aatype, dim=0, batch_dims=0)
    ...
    chi_angle_atoms_mask = utils.batched_gather( params=all_atom_mask, indices=atom_indices, dim=-1, batch_dims=2)
    ...
    #print(chi_angle_atoms_mask)
    #print(chi_angle_atoms_mask.shape)
    chis_mask = chis_mask * (chi_angle_atoms_mask).to(torch.float32)
    ...
    torsion_angles_sin_cos *= torch.tensor( [1., 1., -1., 1., 1., 1., 1.])[None, None, :, None].to(torsion_angles_sin_cos)

    # Create alternative angles for ambiguous atom names.
    chi_is_ambiguous = utils.batched_gather(torch.tensor(residue_constants.chi_pi_periodic), aatype)
    mirror_torsion_angles = torch.cat([torch.ones([num_batch, num_res, 3]), 1.0 - 2.0 * chi_is_ambiguous], dim=-1).to(torsion_angles_sin_cos)
...
def atom14_to_atom37(atom14_data: torch.Tensor,  # (N, 14, ...)
    ...
    assert atom14_data.ndim in [2, 3]
    ...
    if atom14_data.ndim == 2:
        atom37_data *= batch['atom37_atom_exists']
    elif atom14_data.ndim == 3:
...
def atom37_to_atom14(
    ...
    """Convert atom14 to atom37 representation."""
    assert atom37_data.ndim in [2, 3]
    ...
    if atom37_data.ndim == 2:
        atom14_data *= batch['atom14_atom_exists'].to(atom14_data.dtype)
    elif atom37_data.ndim == 3:
        atom14_data *= batch['atom14_atom_exists'][:, :, None].to(atom14_data.dtype)
    return atom14_data
...
def atom37_to_frames(
    ...
    # shape (N, 8, 3)
    residx_rigidgroup_base_atom37_idx = utils.batched_gather(restype_rigidgroup_base_atom37_idx, aatype)

    # Gather the base atom positions for each rigid group.
    base_atom_pos = utils.batched_gather(all_atom_positions, residx_rigidgroup_base_atom37_idx, batch_dims=1)

    # Compute the Rigids.
    gt_frames = r3.rigids_from_3_points(
        point_on_neg_x_axis=r3.vecs_from_tensor(base_atom_pos[:, :, 0, :].cpu()),
        origin=r3.vecs_from_tensor(base_atom_pos[:, :, 1, :].cpu()),
        point_on_xy_plane=r3.vecs_from_tensor(base_atom_pos[:, :, 2, :].cpu())
    )

    # Compute a mask whether the group exists.
    # (N, 8)
    group_exists = utils.batched_gather(restype_rigidgroup_mask, aatype).to(aatype)

    # Compute a mask whether ground truth exists for the group
    gt_atoms_exist = utils.batched_gather(  # shape (N, 8, 3)
        all_atom_mask.to(torch.float32),
        residx_rigidgroup_base_atom37_idx,
        batch_dims=1)
    gt_exists = torch.min(gt_atoms_exist, dim=-1)[0] * group_exists  # (N, 8)
    ...
    residx_rigidgroup_is_ambiguous = utils.batched_gather(restype_rigidgroup_is_ambiguous, aatype)
    residx_rigidgroup_ambiguity_rot = utils.batched_gather(restype_rigidgroup_rots, aatype)
...
#     # back at the end of the function.
#     aatype = torch.reshape(aatype, [-1])
#     all_atom_positions = utils.tree_map(lambda x: torch.reshape(x, [-1, 37]), all_atom_positions)
#     all_atom_mask = torch.reshape(all_atom_mask, [-1, 37])
#
#     # Compute the gather indices for all residues in the chain.
#     # shape (N, 8, 3)
...
#
#     # Gather the base atom positions for each rigid group.
#     base_atom_pos = utils.tree_map(
#             lambda x: utils.batched_gather(x, residx_rigidgroup_base_atom37_idx, batch_dims=1),
#             all_atom_positions)
#
#     # Compute the Rigids.
...
#             residx_rigidgroup_base_atom37_idx,
#             batch_dims=1)
#     gt_exists = torch.amin(gt_atoms_exist, dim=-1) * group_exists  # (N, 8)
#
#     # Adapt backbone frame to old convention (mirror x-axis and z-axis).
#     rots = np.tile(np.eye(3, dtype=np.float32), [8, 1, 1])
#     rots[0, 0, 0] = -1
...
#
#     # Gather the ambiguity information for each residue.
#     residx_rigidgroup_is_ambiguous = utils.batched_gather(restype_rigidgroup_is_ambiguous, aatype).to(aatype.device)
#     ambiguity_rot = utils.batched_gather(restype_rigidgroup_rots, aatype).to(aatype.device)
#     ambiguity_rot = geometry.Rot3Array.from_array(ambiguity_rot)
#
#     # Create the alternative ground truth frames.
#     alt_gt_frames = gt_frames.compose_rotation(ambiguity_rot)
#
#     fix_shape = lambda x: torch.reshape(x, aatype_in_shape + (8,))
#
#     # reshape back to original residue layout
#     gt_frames = utils.tree_map(fix_shape, gt_frames)
...
#     alt_gt_frames = utils.tree_map(fix_shape, alt_gt_frames)
#
#     return {
...
def torsion_angles_to_frames(
    ...
    assert backb_to_global.rot.xx.ndim == 1
    assert torsion_angles_sin_cos.ndim == 3
    ...
    restype_rigid_group_default_frame = torch.tensor(residue_constants.restype_rigid_group_default_frame).to(aatype.device)
    ...
    sin_angles = torch.cat([torch.zeros([num_residues, 1]).to(sin_angles), sin_angles], dim=-1)
    cos_angles = torch.cat([torch.ones([num_residues, 1]).to(cos_angles), cos_angles], dim=-1)
    zeros = torch.zeros_like(sin_angles).to(sin_angles)
    ones = torch.ones_like(sin_angles).to(sin_angles)
    ...
    chi2_frame_to_frame = utils.tree_map(lambda x: x[:, 5], all_frames)
    chi3_frame_to_frame = utils.tree_map(lambda x: x[:, 6], all_frames)
    chi4_frame_to_frame = utils.tree_map(lambda x: x[:, 7], all_frames)

    chi1_frame_to_backb = utils.tree_map(lambda x: x[:, 4], all_frames)
    ...
    all_frames_to_backb = utils.tree_multimap(_concat_frames, all_frames, chi2_frame_to_backb, chi3_frame_to_backb, chi4_frame_to_backb)

    # Create the global frames.
    # shape (N, 8)
    all_frames_to_global = r3.rigids_mul_rigids(utils.tree_map(lambda x: x[:, None], backb_to_global), all_frames_to_backb)
...
def frames_and_literature_positions_to_atom14_pos(
    ...
    restype_atom14_to_rigid_group = torch.tensor(residue_constants.restype_atom14_to_rigid_group).to(aatype.device)
    residx_to_group_idx = utils.batched_gather(restype_atom14_to_rigid_group, aatype)
    residx_to_group_idx = residx_to_group_idx.long()   # modified
    group_mask = F.one_hot(residx_to_group_idx, num_classes=8).to(aatype.device)  # shape (N, 14, 8)

    # r3.Rigids with shape (N, 14)
    map_atoms_to_global = utils.tree_map(lambda x: torch.sum(x[:, None, :] * group_mask, dim=-1), all_frames_to_global)

    # Gather the literature atom positions for each residue.
    # r3.Vecs with shape (N, 14)
    restype_atom14_rigid_group_positions = torch.tensor(residue_constants.restype_atom14_rigid_group_positions).to(aatype.device)
    lit_positions = r3.vecs_from_tensor(utils.batched_gather( restype_atom14_rigid_group_positions, aatype))
    ...
    restype_atom14_mask = torch.tensor(residue_constants.restype_atom14_mask).to(aatype.device)
    mask = utils.batched_gather(restype_atom14_mask, aatype)
    pred_positions = utils.tree_map(lambda x: x * mask, pred_positions)
...
def extreme_ca_ca_distance_violations(
    ...
    has_no_gap_mask = ((residue_index[1:] - residue_index[:-1]) == 1.0).to(torch.float32)
    ca_ca_distance = torch.sqrt(1e-6 + torch.sum(squared_difference(this_ca_pos, next_ca_pos), dim=-1))
...
def between_residue_bond_loss(
    ...
    Dict containing:
      ...
      * 'per_residue_violation_mask': mask denoting all residues with violation
          present.
    """
    assert pred_atom_positions.ndim == 3
    assert pred_atom_mask.ndim == 2
    ...
    c_n_bond_length = torch.sqrt(1e-6 + torch.sum(squared_difference(this_c_pos, next_n_pos), dim=-1))

    # The C-N bond to proline has slightly different length because of the ring.
    next_is_proline = (aatype[1:] == residue_constants.resname_to_idx['PRO']).to(torch.float32)
    gt_length = (
    ...
    ca_c_bond_length = torch.sqrt(1e-6 + torch.sum(squared_difference(this_ca_pos, this_c_pos), dim=-1))
    n_ca_bond_length = torch.sqrt(1e-6 + torch.sum(squared_difference(next_n_pos, next_ca_pos), dim=-1))
    ...
    violation_mask = torch.maximum(F.pad(violation_mask, [0, 1]), F.pad(violation_mask, [1, 0]))
...
def between_residue_clash_loss(
    ...
    Dict containing:
      ...
      * 'per_atom_clash_mask': mask whether atom clashes with any other atom
          shape (N, 14)
    """
    assert atom14_pred_positions.ndim == 3
    assert atom14_atom_exists.ndim == 2
    assert atom14_atom_radius.ndim == 2
    assert residue_index.ndim == 1

    # Create the distance matrix.
    # (N, N, 14, 14)
    dists = torch.sqrt(1e-10 + torch.sum(squared_difference(atom14_pred_positions[:, None, :, None, :], atom14_pred_positions[None, :, None, :, :]), dim=-1))

    # Create the mask for valid distances.
    # shape (N, N, 14, 14)
    dists_mask = (atom14_atom_exists[:, None, :, None] * atom14_atom_exists[None, :, None, :]).float()
    ...
    c_one_hot = F.one_hot(torch.tensor(2), num_classes=14).to(residue_index.device).float()
    n_one_hot = F.one_hot(torch.tensor(0), num_classes=14).to(residue_index.device).float()
    ...
    dists_mask *= (1. - c_n_bonds.float())

    # Disulfide bridge between two cysteines is no clash.
    cys_sg_idx = residue_constants.restype_name_to_atom14_names['CYS'].index('SG')
    cys_sg_one_hot = F.one_hot(torch.tensor(cys_sg_idx), num_classes=14).to(residue_index)
    ...
    # per_atom_clash_mask = torch.maximum( torch.max(clash_mask, dim=[0, 2])[0], torch.max(clash_mask, dim=[1, 3])[0])
    per_atom_clash_mask = torch.maximum( clash_mask.max(2)[0].max(0)[0], clash_mask.max(3)[0].max(1)[0])
...
def within_residue_violations(
    ...
    assert atom14_pred_positions.ndim == 3
    assert atom14_atom_exists.ndim == 2
    assert atom14_dists_lower_bound.ndim == 3
    assert atom14_dists_upper_bound.ndim == 3

    # Compute the mask for each residue.
    # shape (N, 14, 14)
    dists_masks = (1. - torch.eye(14, 14)[None]).to(atom14_atom_exists)
    dists_masks = dists_masks * (atom14_atom_exists[:, :, None] * atom14_atom_exists[:, None, :])

    # Distance matrix
    # shape (N, 14, 14)
    dists = torch.sqrt(1e-10 + torch.sum(squared_difference(atom14_pred_positions[:, :, None, :], atom14_pred_positions[:, None, :, :]), dim=-1))
    ...
    # shape (N, 14)
    per_atom_violations = torch.maximum( torch.max(violations, dim=1)[0], torch.max(violations, dim=2)[0])
...
def find_optimal_renaming(
    ...
    """
    assert atom14_gt_positions.ndim == 3
    assert atom14_alt_gt_positions.ndim == 3
    assert atom14_atom_is_ambiguous.ndim == 2
    assert atom14_gt_exists.ndim == 2
    assert atom14_pred_positions.ndim == 3
    assert atom14_atom_exists.ndim == 2

    # Create the pred distance matrix.
    # shape (N, N, 14, 14)
    pred_dists = torch.sqrt(1e-10 + torch.sum(squared_difference(atom14_pred_positions[:, None, :, None, :], atom14_pred_positions[None, :, None, :, :]), dim=-1))

    # Compute distances for ground truth with original and alternative names.
    # shape (N, N, 14, 14)
    gt_dists = torch.sqrt(1e-10 + torch.sum( squared_difference( atom14_gt_positions[:, None, :, None, :], atom14_gt_positions[None, :, None, :, :]), dim=-1))
    alt_gt_dists = torch.sqrt(1e-10 + torch.sum( squared_difference(atom14_alt_gt_positions[:, None, :, None, :], atom14_alt_gt_positions[None, :, None, :, :]), dim=-1))
...
def frame_aligned_point_error(
    ...
    pair_mask: torch.Tensor,  # shape (num_frames, num_posiitons)
    pair_mask2: torch.Tensor,  # shape (num_frames, num_posiitons)
    length_scale: float,
    l1_clamp_distance: Optional[float] = None,
    epsilon=1e-4,
    use_weights = False) -> torch.Tensor:
    ...
    # print('frames_mask', frames_mask.shape)
    # print(frames_mask)
    # print('positions_mask', positions_mask.shape)
    ...
    local_pred_pos = r3.rigids_mul_vecs(
        utils.tree_map(lambda r: r[:, None], r3.invert_rigids(pred_frames)),
        utils.tree_map(lambda x: x[None, :], pred_positions))

    # Compute array of target positions in the target frames.
    # r3.Vecs (num_frames, num_positions)
    local_target_pos = r3.rigids_mul_vecs(
        utils.tree_map(lambda r: r[:, None], r3.invert_rigids(target_frames)),
        utils.tree_map(lambda x: x[None, :], target_positions))
    ...
    if pair_mask is not None:
        clipped_0_10 = torch.clip(error_dist, 0, 10)
        clipped_0_30 = torch.clip(error_dist, 0, 30)
        error_dist = torch.where(pair_mask, clipped_0_10, clipped_0_30)
    else:
        error_dist = torch.clip(error_dist, 0, 10)

    normed_error = error_dist / length_scale

    if use_weights:
        weights = make_dist_weights(pair_mask, pair_mask2).to(frames_mask.device)
        weights *= torch.unsqueeze(frames_mask, dim=-1)
        weights *= torch.unsqueeze(positions_mask, dim=-2)
        normed_error *= weights
        normalization_factor = torch.sum(weights)
        # normalization_factor = (
        #             torch.sum(frames_mask, dim=-1) * torch.sum(positions_mask, dim=-1))  # N_resolved_res ^ 2
    else:
        normed_error *= torch.unsqueeze(frames_mask, dim=-1)    # [N_res, N_res] * [N_res, 1]
        normed_error *= torch.unsqueeze(positions_mask, dim=-2) # [N_res, N_res] * [1, N_res]
        normalization_factor = ( torch.sum(frames_mask, dim=-1) * torch.sum(positions_mask, dim=-1)) # N_resolved_res ^ 2

    return (torch.sum(normed_error, dim=(-2, -1)) / (epsilon + normalization_factor))


# def make_dist_weights(pair_mask, pair_mask2):
#     n = pair_mask.shape[0]
#     w = torch.zeros((n, n))
#     for i in range(n):
#         for j in range(n):
...
#                 w[i][j] = 1
#                 # t = abs(j - i)
#                 # w[i][j] = max(0.6 + 0.01 * t, 1)
#             elif pair_mask2[i][j] == 1:
#                 w[i][j] = 50
#             else:
#                 w[i][j] = 2.5
#     return w
...
def make_dist_weights(pair_mask, pair_mask2):
    n = pair_mask.shape[0]
    w = torch.full((n, n), 5.0, device=pair_mask.device)  # Initialize with 2.5

    w[pair_mask == 1] = 1.0
    w[(pair_mask == 0) & (pair_mask2 == 1)] = 100.0
...
def get_alt_atom14(aatype, positions, mask):
    ...
    # shape (num_res, 14, 14)
    renaming_transform = utils.batched_gather(torch.tensor(RENAMING_MATRICES), aatype)

    positions = utils.tree_map(lambda x: x[:, :, None], positions)
    alternative_positions = utils.tree_map(lambda x: torch.sum(x, dim=1), positions * renaming_transform)

    # Create the mask for the alternative ground truth (differs from the
    # ground truth mask, if only one of the atoms in an ambiguous pair has a
    # ground truth position)
    alternative_mask = torch.sum(mask[..., None] * renaming_transform, dim=1)
