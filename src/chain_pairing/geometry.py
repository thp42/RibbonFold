...
def kabsch_rotation(P, Q):
    ...
    V, _, W = torch.linalg.svd(C.float())
    d = (torch.linalg.det(V.float()) * torch.linalg.det(W.float())) < 0.0
    #V = V.to(C)
    #W = W.to(C)
    #d = d.to(C)
...
def get_optimal_transform_v2(
    ...
    v, _, w = torch.linalg.svd(c.float())  # [*, 3, 3]
    d = (torch.linalg.det(v.float()) * torch.linalg.det(w.float()) >= 0.0).type(v.dtype) * 2.0 - 1.0  # [*]
    #v = v.to(c)
    #w = w.to(c)
    #d = d.to(c)
