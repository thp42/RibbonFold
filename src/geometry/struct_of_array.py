...
@property
def get_ndim(instance):
    """Returns number of dimensions for given instance of dataclass."""
    first_field = dataclasses.fields(instance)[0]
    num_trailing_dims = first_field.metadata.get('num_trailing_dims', None)
    value = getattr(instance, first_field.name)
    if num_trailing_dims:
        return len(value.shape[:-num_trailing_dims])
    else:
    ...
    if dtype is not None:
        ...
        for array, field in zip(arrays, array_fields):
            ...
            if num_trailing_dims:
                ...
                msg = (f'field {field} should have number of trailing dims {num_trailing_dims}')
                assert len(array_shape) == len(first_shape) + num_trailing_dims, msg
            else:
                field_shape = array.shape

            shape_msg = (f"Stripped Shape {field_shape} of field {field} doesn't match shape {first_shape} of field {first_field}")
...
def custom_flatten(instance, parent_node):
    """Flatten Struct of Array instance."""
    tree = parent_node.data['tree']
    
    ################# >>>>>>>>> tag=type(instance) ?? <<<<<<<<<<<<<
    node = tree.create_node(tag=type(instance), parent=parent_node.identifier, data={'tree': tree})
    ...
    for array_like in array_likes:
        flat_array_like, inner_treedef = pytree.tree_flatten(array_like)
    ...
    node.data['other_data'] = (inner_treedefs, metadata, num_arrays)
    return flat_array_likes, node
...
def get_fields(cls_or_instance, filterfn, return_values=False):
    ...
    if return_values:
        return { field.name: getattr(cls_or_instance, field.name) for field in fields }
    else:
        return fields


def get_array_fields(cls, return_values=False):
    return get_fields(cls, lambda x: not x.metadata.get('is_metadata', False), return_values=return_values)


def get_metadata_fields(cls, return_values=False):
    return get_fields(cls, lambda x: x.metadata.get('is_metadata', False), return_values=return_values)
...
class StructOfArray:
    ...
    def __call__(self, cls):
        ...
        def custom_unflatten(node, data):
            assert isinstance(node, treelib.node.Node)
            #assert isinstance(self, node.tag)
            assert isinstance(data, list)
            tree = node.data['tree']
            
            inner_treedefs, metadata, num_arrays = node.data['other_data']
            ...
            for num_array, inner_treedef, array_field in zip(num_arrays, inner_treedefs, array_fields):
                value_dict[array_field] = pytree.tree_unflatten(inner_treedef, data[array_start:array_start + num_array])
            ...
            return new_cls(**value_dict), consumed

        pytree.register_pytree_node(nodetype=new_cls, flatten_func=custom_flatten, unflatten_func=custom_unflatten)
        return new_cls
