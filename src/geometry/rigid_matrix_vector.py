...
class Rigid3Array:
    ...
    def compose_rotation(self, other_rotation):
        ...
        trans = utils.tree_map(lambda x: torch.broadcast_to(x, rot.shape), self.translation)
        return Rigid3Array(rot, trans)

    @classmethod
    def identity(cls, shape, dtype=torch.float32) -> Rigid3Array:
    ...
    def to(self, other):
        ...
        if device:
            rotation = utils.tree_map(lambda x: x.to(device), rotation)
            translation = utils.tree_map(lambda x: x.to(device), translation)
        if dtype:
            rotation = utils.tree_map(lambda x: x.to(dtype), rotation)
            translation = utils.tree_map(lambda x: x.to(dtype), translation)
