...
class Vec3Array:
    ...
    def __add__(self, other: Vec3Array) -> Vec3Array:
        return pytree.tree_multimap(lambda x, y: x + y, self, other)

    def __sub__(self, other: Vec3Array) -> Vec3Array:
        return pytree.tree_multimap(lambda x, y: x - y, self, other)

    def __mul__(self, other: Float) -> Vec3Array:
        return pytree.tree_map(lambda x: x * other, self)
    ...
    def norm(self, epsilon: float = 1e-6) -> Float:
        ...
        norm2 = self.dot(self)
        if epsilon:
            norm2 = torch.maximum(norm2, torch.tensor(epsilon**2).to(norm2))
    ...
    @classmethod
    def zeros(cls, shape, dtype=torch.float32):
        """Return Vec3Array corresponding to zeros of given shape."""
        return cls( torch.zeros(shape, dtype=dtype), torch.zeros(shape, dtype=dtype), torch.zeros(shape, dtype=dtype) )

    def to_array(self) -> torch.Tensor:
        return torch.stack([self.x, self.y, self.z], dim=-1)

    def to(self, other) -> Vec3Array:
        return pytree.tree_map(lambda x: x.to(other), self)
    ...
    def __getstate__(self):
        return (VERSION,[np.asarray(self.x),
                         np.asarray(self.y),
                         np.asarray(self.z)])
...
def square_euclidean_distance(vec1: Vec3Array, vec2: Vec3Array, epsilon: float = 1e-6) -> Float:
    ...
    if epsilon:
        distance = torch.maximum(distance, torch.tensor(epsilon).to(distance))
    return distance
...
def random_gaussian_vector(shape, key, dtype=torch.float32):
    vec_array = torch.randn(shape + (3,), dtype)
